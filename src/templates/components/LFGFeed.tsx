...
import ENV from '../../env';
import Environment from '../../env_interface';
...
export class Feed extends React.Component<FeedProperties, FeedState> {
    private colorMap = {
        0: {
    ...
    public constructor(props: FeedProperties) {
        ...
        this.state = {
            setup: props.lfgs !== undefined && props.lfgs.length > 0 ? false : true,
            lfgs: props.lfgs !== undefined && props.lfgs.length > 0 ? props.lfgs : [],
            refreshing: false,
        };
        ...
        axios
            ...
            .then((response) => {
                ...
                if (
                    ...
                    /* Throttle the finalizing by the difference of our end and start time
                    Max amount of time allowed is 1.5 seconds  delay
                    Minimum amount of time allowed is 0 seconds delay 
                    console.log(
                        timeStart,
                        timeEnd,
                        timeEnd - timeStart,
    ...
    public renderSetup() {
        ...
        return (
            <div className="animate-pulse feed p-4 sticky top-0" data-name={this.props.name}>
                <h3 className="text-1xl  md:text-2xl  lg:text-3xl  font-sans font-bold uppercase mb-2 dark:text-white rounded-lg p-2 py-4 dark:bg-gradient-to-t dark:bg-slate-900 dark:from-black dark:to-slate-800 transition-all duration-300">
                    {'#' + this.props.name}
                </h3>
    ...
    public renderFeed() {
        ...
        return (
            <div
                className={'feed p-4 sticky top-0 ' + (this.state.refreshing === true ? 'animate-pulse' : '')}
            ...
            >
                <h3 className="text-1xl  md:text-2xl  lg:text-3xl font-sans font-bold uppercase mb-2 dark:text-white rounded-lg p-2 py-4 dark:bg-gradient-to-t dark:bg-slate-900 dark:from-black dark:to-slate-800 transition-all duration-300">
                    ...
                    >
                        <i className={'fas fa-arrows-rotate ' + (this.state.refreshing ? 'animate-spin' : '')}></i>
                    </button>
                ...
                <div className="activities">
                    {this.state.lfgs.map((activity, index) => (
                        ...
                        >
                            ...
                            <p className="p-4 bg-slate-600 text-white rounded-t">{activity.description}</p>
