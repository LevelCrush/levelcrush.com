...
export class RaidGuideManager {
    ...
    public async htmlify() {
        if (this.guide && ENV.server && ENV.server.assets) {
            const mediaRegex = /\[\[\{\{\s([0-9a-zA-Z.]+)\s\}\}\]\]/g;
            const assetPath = ENV.server.assets;
            ...
            for (let encounterID in this.guide.encounters) {
                ...
                for (let sectionID in this.guide.encounters[encounterID].sections) {
                    ...
                    for (let i = 0; i < mediaChunks.length; i++) {
                        ...
                        if (typeof this.guide.assets[content] !== 'undefined') {
                            ...
                            if (targetPath.trim().length > 0) {
                                ...
                                const urlPath = '/assets/guides/destiny2/votd/' + content.trim() + extension;

                                // for now we will put the img element inside a picture element for future case
                                contentChunks.push('<picture><img src="' + urlPath + '" /></picture>');
                            }
                        } else {
                            ...
                            do {
                                ...
                                let newContent = this.findUnderlineInstance(content, currentIndex);
                            ...
                            do {
                                ...
                                let newContent = this.findStrikeThroughInstance(content, currentIndex);
                                foundChange = newContent.changed;
                                if (foundChange) {
                                    content = newContent.output;
                                    currentIndex = newContent.index;
                    ...
                    (this.guide.encounters[encounterID].sections[sectionID] as any)['contentHtml'] =
                        contentChunks.join('');
    ...
    private linkToYouTubeEmbed(input: string) {
        const regex = /(<a(.*)\>http(s?)\:\/\/([wW\.]+)?youtu([a-zA-z\.]+)\/(watch\?v=)?([0-9A-Za-z\-]+)\<\/a>)/;
        const subst = `$1<iframe class="youtube-player" type="text/html" width="100%" height="600"  src="https://www.youtube.com/embed/$7?autoplay=0"  frameborder="0"></iframe>`;
    ...
    private swapTokenInstance(input: string, index: number, token: string, wrapStart: string, wrapEnd: string) {
        ...
        if (tokenStart !== -1) {
            ...
            if (tokenEnd !== -1) {
                ...
                let newString = originalString.replace(token, wrapStart).replace(token, wrapEnd);
    ...
    private fileExists(pathToFile: string): Promise<{ path: string; result: boolean }> {
    ...
    public async pull() {
        ...
        try {
            ...
            if (
                ...
                this.guide = JSON.parse(response.data['response']['feed']['data']) as RaidGuide;
            }
        } catch (err) {
            console.log(err);
    ...
    public async prerender() {
        ...
        if (ENV.server && ENV.server.assets && ENV.server.guideCache && this.guide) {
            await this.htmlify();

            const guideDirPath = ENV.server.guideCache;
    ...
    public async downloadGuideAssets() {
        ...
        if (ENV.server && ENV.server.assets) {
            const assetPath = ENV.server.assets;
            const guidePath = path.join(assetPath, 'guides');
            const gamePath = path.join(guidePath, 'destiny2');
            const raidGuidePath = path.join(gamePath, this.baseName);
